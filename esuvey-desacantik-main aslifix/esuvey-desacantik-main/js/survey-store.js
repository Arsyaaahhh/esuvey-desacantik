...
const SurveyStore = {
  ...
  // Simpan titik lokasi (GPS) hasil "Ambil Lokasi Saat Ini" di step Dokumentasi.
  // Format: {lat, lng, akurasi (meter), waktu (ISO string)}
  saveLocation(loc) {
    localStorage.setItem("gcdc_survey_location", JSON.stringify(loc));
  },

  getLocation() {
    try {
      return JSON.parse(localStorage.getItem("gcdc_survey_location")) || null;
    } catch (e) {
      return null;
    }
  },
  ...
  clearLocation() {
    localStorage.removeItem("gcdc_survey_location");
  },
