...
const SHEET_HEADERS = {
  Rumah_Tangga: [
    ...
    "catatan", "foto_links", "latitute", "longitude", "maps_link",
  ],
  Individu_Anggota: [
    "timestamp", "pic_nama", "pic_telp", "pic_email",
    "nama_kepala_keluarga", "nik_kepala_keluarga", "no_kk",
...
function doPost(e) {
  try {
    ...
    Logger.log("latitute: " + data.latitute + " | longitude: " + data.longitude + " | maps_link: " + data.maps_link);
...
function savePhotosToDrive(photos, sheetName, data) {
  ...
  photos.forEach((p) => {
    try {
      const base64 = p.base64.split(",").pop();
